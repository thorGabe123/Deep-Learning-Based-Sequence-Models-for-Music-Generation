    ...
    for band in band_folders:
        ...
        while generated_files < B:
            ...
            for src, trg, meta in test_dataloader:
                break
            A, _ = src.shape
            if generated_files + A > B:
                src = src[:B-generated_files]
                meta = meta[:B-generated_files]
            A, _ = src.shape
            if args.no_metadata:
            ...
            for i in range(A):
                ...
                if args.combined_path:
                    ...
                    processing.note_to_midi(
                        decoded_notes_mamba,
                        os.path.join(combined_output_dir, f"generated_{band}_mamba_{generated_files+i}.mid")
                    )
                    processing.note_to_midi(
                        decoded_notes_xlstm,
                        os.path.join(combined_output_dir, f"generated_{band}_xlstm_{generated_files+i}.mid")
                    )
                    processing.note_to_midi(
                        decoded_notes_transformer,
                        os.path.join(combined_output_dir, f"generated_{band}_transformer_{generated_files+i}.mid")
                    )
                else:
                    if args.mamba:
                        processing.note_to_midi(
                            decoded_notes_mamba,
                            os.path.join(mamba_output_dir, f"generated_{band}_mamba_{generated_files+i}.mid")
                        )
                    if args.xlstm:
                        processing.note_to_midi(
                            decoded_notes_xlstm,
                            os.path.join(xlstm_output_dir, f"generated_{band}_xlstm_{generated_files+i}.mid")
                        )
                    if args.transformer:
                        processing.note_to_midi(
                            decoded_notes_transformer,
                            os.path.join(transformer_output_dir, f"generated_{band}_transformer_{generated_files+i}.mid")
                        )
            generated_files += A