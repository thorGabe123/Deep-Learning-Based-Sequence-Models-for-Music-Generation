...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Adjust sequence length (truncate or pad)
            ...
            if cc.config.values.end_of_seq:
                ix = len(sequence) - seq_len_extra - 1 #Only for testing the final part of a sequence
            elif cc.config.values.start_of_seq:
                ix = 0
            else:
                ix = random.randint(0, (len(sequence) - seq_len_extra))
            sequence = sequence[ix: ix + seq_len_extra]

        sequence = torch.tensor(sequence, device=cc.config.values.device, dtype=torch.long)
        if cc.config.values.augmentation:
...
class DatasetLoader:
    ...
    def get_dataloader_full(self, shuffle=False):
        """
        Get a DataLoader for the entire dataset.
        """
        return DataLoader(
            self.dataset,
            batch_size=self.batch_size,
